...
- **デバウンス後の LLM 整形**: ログが静止してから `--debounce-ms` 経過後に、スナップショットを Ollama のチャット API に渡し、**音声向けの日本語テキスト**を受け取る。進行中リクエストは **`AbortController` で中止**できること。実装上の既定の `--debounce-ms` は README と同様 **20000 ms** とする。
