...
CLI タイプのコーディングエージェントは、ターミナル上で長文の標準出力を返すことがある。本ツールは、その出力を**ログファイルに集約**し、別プロセスでファイルを**監視**する。既定ではログからテキストを抽出したうえで **ローカルの Ollama（既定モデル `gemma4:e4b`）へ送信し、音声読み上げ向けに整形した日本語を macOS の音声合成（`say`）で読み上げる**。ターミナルの装飾・入力途中のノイズを抑え、要点だけを聞き取りやすくすることを目的とする。**`--legacy-line-speak` を指定した場合**は Ollama を使わず、正規化したログ行を従来どおりそのまま読み上げる。
...
- **出力**: `say` による音声。既定モードでは各 Ollama 呼び出しについて、送信するチャットの **system / user メッセージ本文**およびモデルが返した**読み上げ用テキスト**を標準出力へ書き出す（観測・デバッグ用）。運用ログ・進捗は引き続き主に標準エラーへ出力される。
...
- **ローリングスナップショット**: フィルタ後に得られた有意な行を末尾優先で連結し、`--snapshot-max-chars` でメモリ上の連結長に上限を設ける。
- **LLM 向けプロンプトの絞り込み**: Ollama に送る直前に、スナップショット本文から **末尾 `--llm-tail-lines` 行**に絞り、その結果が **`--llm-prompt-max-chars` を超える場合は末尾から最大その文字数**に切り詰める（いずれも正の整数、既定は README に合わせる）。全体ログではなく直近の要約に相当する範囲のみモデルへ渡す。
- **デバウンス後の LLM 整形**: ログが静止してから `--debounce-ms` 経過後に、（上記で絞ったテキストを） Ollama のチャット API に渡し、**音声向けの日本語テキスト**を受け取る。進行中リクエストは **`AbortController` で中止**できること。実装上の既定の `--debounce-ms` は README と同様 **20000 ms** とする。
- **Ollama チャットのタイムアウト**: `/api/chat` は **`--ollama-timeout-ms`**（ミリ秒、正の整数）以内に完了しない場合に打ち切り、呼び出し側が長時間無応答とログ追記による中止を区別できるようにする。既定値は README と同様 **180000 ms**。タイムアウト時はユーザー向けに明示され、プロセスはエラー終了してよい（実装に準拠）。
...
- **オプション**  
  ...
  - `--ollama-model <name>`: 利用するモデル名（既定は実装上 `gemma4:e4b`）。  
  - `--debounce-ms <ms>`: ログ静止後、Ollama を呼ぶまで待つミリ秒（正の整数）。デフォルトは実装上 20000 ms。  
  - `--ollama-timeout-ms <ms>`: `/api/chat` の応答待ち上限（正の整数）。デフォルトは実装上 180000 ms。  
  - `--snapshot-max-chars <n>`: ローリングスナップショット連結の最大文字数（正の整数）。既定は実装上 48000。  
  - `--llm-tail-lines <n>`: Ollama に送る末尾の最大行数（正の整数）。既定は実装上 40。  
  - `--llm-prompt-max-chars <n>`: 行で絞ったあとのプロンプト最大文字数（正の整数、末尾優先で切り詰め）。既定は実装上 12000。  
...
- **レイテンシ**: 読み上げ開始までの遅れは `--poll-interval`、`--debounce-ms`、Ollama の推論時間、`say` の処理時間に依存する。`/api/chat` が `--ollama-timeout-ms` を超えて完了しない場合は打ち切られる。
- **信頼性**: 監視中の一時的な I/O 失敗は標準エラーに記録し、可能ならポーリングを継続する。
- **セキュリティ**: ログはユーザーのマシン上で処理される。既定モードでは**ローカルの Ollama API**へスナップショットが送信され、モデルにログ内容が渡る。第三者クラウドへの送信は要件としないが、`--ollama-url` を変更した場合はそのエンドポイントへの送信となる。機密を含むログを扱う場合はモデルの実行環境と保管ポリシーをユーザーが判断する。`--legacy-line-speak` では Ollama を経由しない。既定モードではプロンプトと応答が標準出力にも出るため、共有ターミナルやログのリダイレクト時の漏えいに注意する。
